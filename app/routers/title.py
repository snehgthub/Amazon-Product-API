...
router = APIRouter(prefix="/product", tags=["Product Title"])


@router.post(
    "/title", status_code=status.HTTP_200_OK, response_model=schemas.ProductTitle
)
def get_title_from_url(
    ...
    if not product_asin:
        raise HTTPException(
            ...
            detail="Cannot retrieve Product Title: no product found with the given URL",
        )
...
@router.get(
    "/{asin}/title", status_code=status.HTTP_200_OK, response_model=schemas.ProductTitle
)
def get_title_by_asin(
