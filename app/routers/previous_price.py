...
router = APIRouter(prefix="/product", tags=["Product Previous Price"])


@router.post(
    "/price/previous",
    status_code=status.HTTP_200_OK,
    response_model=schemas.ProductPreviousPrice,
    responses={
        status.HTTP_404_NOT_FOUND: {"model": schemas.HTTPError},
    },
)
def get_previous_price_from_url(
    ...
    if not product_asin:
        raise HTTPException(
            ...
            detail="Cannot retrieve Previous Price: no product found with the given URL",
        )
...
def get_previous_price_by_asin(
