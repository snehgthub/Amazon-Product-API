...
router = APIRouter(prefix="/product", tags=["Product Current Price"])


@router.post(
    "/price/current",
    status_code=status.HTTP_200_OK,
    response_model=schemas.ProductCurrentPrice,
    responses={
        status.HTTP_404_NOT_FOUND: {"model": schemas.HTTPError},
    },
)
def get_current_price_from_url(
    ...
    if not product_asin:
        raise HTTPException(
            ...
            detail="Cannot retrieve Current Price: no product found with the given URL",
        )
