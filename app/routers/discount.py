...
router = APIRouter(prefix="/product", tags=["Product Discount"])


@router.post(
    "/discount",
    status_code=status.HTTP_200_OK,
    response_model=schemas.ProductDiscount,
    responses={
        status.HTTP_404_NOT_FOUND: {"model": schemas.HTTPError},
    },
)
def get_discount_from_url(
    ...
    if not product_asin:
        raise HTTPException(
            ...
            detail="Cannot retrieve Discount: no product found with the given URL",
        )
