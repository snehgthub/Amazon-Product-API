...
router = APIRouter(prefix="/product", tags=["Product Ratings Count"])


@router.post(
    "/ratings-count",
    status_code=status.HTTP_200_OK,
    response_model=schemas.ProductRatingsCount,
    responses={
        status.HTTP_404_NOT_FOUND: {"model": schemas.HTTPError},
    },
)
def get_product_ratings_count_from_url(
    ...
    if not product_asin:
        raise HTTPException(
            ...
            detail="Cannot retrieve Ratings Count: no product found with the given URL",
        )
